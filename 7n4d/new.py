...
if __name__ == '__main__':
    ...
    for episode in range(TOTAL_EPISODES+800000):
        ...
        # === Multi-stage learning-rate control logic ===
        if agent.scheduler is not None and len(agent.memory) > agent.batch_size:
            if episode < TOTAL_EPISODES:
                # Stage 1: for the first 200,000 episodes, decay normally with cosine annealing
                agent.scheduler.step()
            elif episode == TOTAL_EPISODES + 400000:
                # Stage 3 start: at episode 600,000, manually halve the learning rate
                # (Stage 2 is 200,000 to 600,000; do nothing and keep it constant)
                for param_group in agent.optimizer.param_groups:
                    param_group['lr'] *= 0.1
                print(f"\n[LR Adjustment] Episode {episode}: Learning rate has been halved; preparing for the final 400,000 fine-tuning episodes!")
