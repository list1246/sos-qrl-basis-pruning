...
if __name__ == '__main__':
    ...
    generator = SOSDataGenerator(num_vars=2, degree=6)
    ...
    agent = DoubleDQNAgentPER(generator.coeff_dim, generator.mask_dim, device=DEVICE, base_dim=512, embed_dim=16, total_episode=TOTAL_EPISODES)
    ...
    for episode in range(TOTAL_EPISODES):
        ...
        if (episode + 1) % 100 == 0:
            ...
            print(
                f"Ep {episode + 1:<5} | "
                f"Train Avg R: {avg_r:6.2f} | "
                f"Eps: {agent.epsilon:.3f} | "
                f"LR: {current_lr:.2e} | "  # <--- Added this line, using scientific notation (e.g. 1.00e-04)
                f"Step Gap: {avg_train_gap:4.1f}"
            )
