...
if __name__ == '__main__':
    ...
    generator = SOSDataGenerator(num_vars=3, degree=8)

    # === 修改点 2：初始化两个环境 ===
    # 训练环境：使用 train.json，限制采样 10000 条用于训练 (train_data_size=10000)
    env = SOSPruningEnvGT(generator, DATASET_TRAIN,
                          device=DEVICE, train_data_size=10000)

    # 测试环境：使用 test.json，设一个很大的数以加载全部测试集 (train_data_size=100000)
    # 注意：env.py 的逻辑是 "如果数据量 > target_size 则采样"，设大一点就能全量加载
    test_env = SOSPruningEnvGT(
        generator, DATASET_TEST, device=DEVICE, train_data_size=100000)

    agent = DoubleDQNAgentPER(generator.coeff_dim, generator.mask_dim,
                              device=DEVICE, base_dim=1024, embed_dim=32, total_episode=TOTAL_EPISODES)
    ...
    for episode in range(TOTAL_EPISODES+100000):
        ...
        if (episode + 1) % EVAL_INTERVAL == 0:
            ...
            if len(eval_test_gaps) > 1 and gap_test < min(eval_test_gaps[:-1]):
                print(
                    f"[Save] New best TEST gap: {gap_test:.4f}. Saving model...")
                torch.save(agent.policy_net.state_dict(
                ), f"./model/{os.path.splitext(os.path.basename(__file__))[0]}.pth")
            print("-" * 30 + "\n")
